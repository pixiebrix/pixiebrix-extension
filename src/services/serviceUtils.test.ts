...
describe("extractServiceIds", () => {
  it("errors by default if not found", () => {
    expect(() => extractServiceIds({})).toThrow();
  });

  it("can suppress error", () => {
    expect(extractServiceIds({}, { suppressNotFoundError: true })).toEqual([]);
  });

  it("can extract from ref", () => {
    expect(extractServiceIds(SHEET_SERVICE_SCHEMA)).toEqual(["google/sheet"]);
  });

  it("ignores non-service ref", () => {
    expect(extractServiceIds({ $ref: "tacos" })).toEqual([]);
  });
